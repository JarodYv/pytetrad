...
from search.PcAll import PcAll
...
class Pc(Algorithm, TakesInitialGraph, HasKnowledge, TakesIndependenceWrapper):
    ...
    def search(self, dataset: DataModel, **parameters) -> Graph:
        if parameters["number_resampling"] < 1:
            ...
            search.set_conflict_rule(PcAll.ConflictRule.PRIORITY)
            search.set_verbose(parameters.get("verbose"))
            search.set_collider_discovery(PcAll.ColliderDiscovery.FAS_SEPSETS)
            search.set_fas_type(PcAll.FasType.REGULAR)
            search.set_concurrent(PcAll.Concurrent.NO)
    ...
    def get_comparison_graph(self, graph: Graph) -> Graph:
        return SearchGraphUtils.patternForDag(EdgeListGraph(graph))

    def get_description(self) -> str:
        algorithm_desc = "" if not self.algorithm else f" with initial graph from {self.algorithm.get_description()}"
        return f"PC (\"Peter and Clark\"), Priority Rule, using {self.test.get_description()} {algorithm_desc}"
